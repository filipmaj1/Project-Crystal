...
using Crystal.Mahjong.Packets.GamePackets.Receive;
using Crystal.Mahjong.Tables;
using System;
...
namespace Crystal.Mahjong.Models
{
    public class Table
    {
        ...
        public ulong CreateTableId(ulong roomId, uint tableNumber)
        {
            ...
            polIdData[6] = hex[0];
            polIdData[7] = hex[1];
            ulong tableId = SqCrypto.PolProDataToPolId(new(polIdData), 0, 3);
        ...
        public Table(ulong roomId, uint tableNumber)
        {
            ...
            TableLobbyChannel = new PolIrcClient($"Mj-Table-{Number}", Id, OnReceiveMsg);
        ...
        private int GetMasterIndx()
        {
            for (int i = 0; i < NumMembers; i++)
            {
                if (Members[i].Id == MasterPolId)
                    return i;
            }
            return 0;
        }
        ...
        private void OnReceiveMsg(string from, string data)
        {
            ...
            {
                MjPacket gamePkt = MjPacket.FromIrcStr(mgPacket.GetGamePacketData());
                ...
                switch ((int)gamePkt.Header.Command)
                {
                    case (byte)MjConstants.Opcodes.MjPLAYREQ:
                        PlayReq playReq = MemoryMarshal.Cast<byte, PlayReq>(gamePkt.Data)[0];

                        // Add player
                        byte answer;
                        TablePlayer enteringPlayer = new TablePlayer(playReq.PlayerId, playReq.PlayerName, 1, 1, 1);
                        if (enteringPlayer != null)
                            answer = (byte) AddPlayer(enteringPlayer);
                        else
                            answer = 11;


                        // Send response
                        ReserveAck reserveAck = new()
                        {
                            resultCode = 1,
                            resultData = 1
                        };
                        byte[] reserveAckBytes = new byte[ReserveAck.SIZE];
                        MemoryMarshal.Write(reserveAckBytes, ref reserveAck);
                        MjPacket responsePkt = new(
                            0,
                            target: Mg.MjKey(SqCrypto.PolProDataToPolId(from, 0, 0)),
                            command: MjConstants.Opcodes.MjRESERVEACK,
                            fifoChan: gamePkt.Header.fifoChannel,
                            seq: gamePkt.Header.SequenceNum,
                            data: reserveAckBytes);
                        TableLobbyChannel.Notice(from, responsePkt.ToIrcStr());

                        Program.Log.Debug($"Game Packet Send - Opcode: {responsePkt.Header.Command}, Sequence: {responsePkt.Header.SequenceNum}.\n{Utils.ByteArrayToHex(responsePkt.Data)}");

                        UpdateTable();
                        break;
                    case (byte)MjConstants.Opcodes.MjPLAYCANCEL:
                        ReserveAck reserveAck2 = new()
                        {
                            resultCode = 5,
                            resultData = 1
                        };
                        byte[] reserveAckBytes2 = new byte[ReserveAck.SIZE];
                        MemoryMarshal.Write(reserveAckBytes2, ref reserveAck2);


                        MjPacket responsePkt3 = new(
                            0,
                            target: Mg.MjKey(SqCrypto.PolProDataToPolId(from, 0, 0)),
                            command: MjConstants.Opcodes.MjRESERVEACK,
                            fifoChan: gamePkt.Header.fifoChannel,
                            seq: gamePkt.Header.SequenceNum,
                            data: reserveAckBytes2);


                        TableLobbyChannel.Notice(from, responsePkt3.ToIrcStr());
                        break;
                    case (byte)MjConstants.Opcodes.MjTBLCONFALL:
                        MasterCmdAck masterCmdAck = new()
                        {
                            Result = 1
                        };
                        byte[] masterCmdAckBytes = new byte[MasterCmdAck.SIZE];
                        MemoryMarshal.Write(masterCmdAckBytes, ref masterCmdAck);

                        MjPacket responsePkt2 = new(
                            0,
                            target: Mg.MjKey(SqCrypto.PolProDataToPolId(from, 0, 0)),
                            command: MjConstants.Opcodes.MjMASTERCMDACK,
                            fifoChan: gamePkt.Header.fifoChannel,
                            seq: gamePkt.Header.SequenceNum,
                            data: masterCmdAckBytes);


                        Program.Log.Debug($"Game Packet Send - Opcode: {responsePkt2.Header.Command}, Sequence: {responsePkt2.Header.SequenceNum}.\n{Utils.ByteArrayToHex(responsePkt2.Data)}");

                        TableLobbyChannel.Notice(from, responsePkt2.ToIrcStr());
                        break;
                }
            }
        }
        ...
        {
            string gameDataStr = $"{Number:D5},{0:D5},{3:D3},{0:D3},{ReservationTimer:D3},{(CanSpectate ? 1 : 0):D1},{0:D2},{0:D2},{GetMasterIndx():D1},{(HasPassword ? 1 : 0):D1},{(RulesSetByMaster ? 1 : 0):D1},{CommentId:D1},{0:D3},{Restrictions:D1}";
        ...
        {
            return new(Id, 0, 0, 1, State, IrcChannel, GameData);
        }
    }
}
