...
{
    public class FmoBlowfish
    {   
        private int N = 16;
        private UInt32[] P;
        private UInt32[,] S;

        public unsafe struct BlowfishContext
        {
            public fixed uint P[18];
            public fixed uint S[1024];
        }
        ...
        public FmoBlowfish(byte[] key, int keySize = 16)
        {
            InitializeBlowfish(key, keySize, P_values, S_values);
        }
        ...
        public void BlowfishDecipher(ref UInt32 xl, ref UInt32 xr)
        {
            UInt32 temp;
            Int32 i;
        ...
        private int InitializeBlowfish(byte [] key, int keySize, byte[] pTable, byte[] sTable)
        {
            N = 16;
            P = new uint[N + 2];
            S = new UInt32[4, 256];
            ...
            Buffer.BlockCopy(pTable, 0, P, 0, pTable.Length);
            Buffer.BlockCopy(sTable, 0, S, 0, sTable.Length);
